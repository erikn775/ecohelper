...
class Appliances extends React.Component{
    ...
    componentDidMount(){
        fetch(`http://127.0.0.1:3000/appliance/${this.props.unit}`)
    ...
    addToCost = (event) => {
        ...
        if(event.target.parentElement.className === 'appliance-container'){
            event.target.parentElement.style = 'border-style: solid; border-color: lightgreen; border-width: 5px;';
        }
        if(event.target.className === 'appliance-container'){
    ...
    render(){
        ...
        return(
            <div onClick={clicked ? this.subtractCost : this.addToCost} className="appliance-container">
                <h3 className="appliance-title">{this.state.energyLevel} Appliances</h3>
...
export default connect(null, mapDispatchToProps)(Appliances);